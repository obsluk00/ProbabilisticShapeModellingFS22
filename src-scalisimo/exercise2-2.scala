...
(0 until 50).foreach{i :Int =>
  ...
  val sample = lowRankGP.sampleAtPoints(reference)
  val landmarks = referenceLandmarks.map(pId => Landmark(s"${pId.id}", transformFromDefField(sample)(pId.point)))
...
writeCSV(new File("datasets/project-data/distancesGPModel.csv"), ourMeasurements)
